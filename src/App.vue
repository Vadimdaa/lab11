<template>
  <div class="container">
  <header>
    <h1>МЕГАДОСКА ВАДИМА</h1>
    <b-button class="button" @click="toggleTheme">Смена темы</b-button>
  </header>
    <b-button  class="col-2" variant="danger" @click="open=true">Создать</b-button>
    ...
    <div class="row mt-3">
      <div class="col-md-4">
        <div class="p-2 alert alert-secondary backlog">
          <h3 class="text-center">Планируется {{arrBacklog.length}}</h3>
      ...
      <div class="col-md-4">
        <div class="p-2 alert alert-primary inprog">
          <h3 class="text-center">Выполняется {{arrInProgress.length}}</h3>
      ...
      <div class="col-md-4">
        <div class="p-2 alert alert-success done">
          <h3 class="text-center">Выполнено {{arrDone.length}}</h3>
  ...
  mounted() {
   ...
   data(){
    return{
      ...
      arrBacklog: [
        {name: 'Сдать веб', date: '12.7.2021', level: 3, description: 'очень важная задача', number: 1}
        ],
      arrInProgress:[],
      arrDone:[],
      open: false
